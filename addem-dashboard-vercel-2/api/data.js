...
/* ---------- Origination ("Draws" tab, formerly "Sheet 3" / Portfolio Draws) ----------
   Layout (1-indexed rows in the sheet):
     - Header rows 1-17 (dates in the row with the most Date cells)
     - Rows 18-60  : Draw Forecast by borrower (the main chart) — bounded by the first
                     "TOTAL" row (row 61) so we never bleed into the per-lender section
     - Rows 63-199 : per-lender breakdown in three blocks, each starting with a header
                     ("Pledged to Variant", "Pledged to Covalto", "Non-Pledged") and ending
                     with its own "TOTAL" row. Each block repeats the same borrower list, so
                     it must be parsed in isolation from the main forecast and from each other.
   ------------------------------------------------------------------------------------ */
function normName(s){
...
/* Locate the date row + month columns + label column once and reuse for every block. */
function drawGeometry(A) {
  ...
  return { dateRow, cols, labelCol, unitCol: labelCol + 1, monthsAll: cols.map((c) => ym(dr[c])) };
}

/* Accumulate borrower x month over a [rowStart, rowEnd) window (0-indexed array rows),
   skipping header/total rows and any row whose unit cell isn't MXN/USD. Trims leading and
   trailing all-zero months so each chart spans only its productive range. */
function accumulateDraws(A, G, rowStart, rowEnd) {
  const { cols, labelCol, unitCol, monthsAll } = G;
  const perB = {};
  for (let r = rowStart; r < Math.min(rowEnd, A.length); r++) {
    const lab = (A[r] || [])[labelCol];
    if (!(typeof lab === "string" && lab.trim())) continue;
    const t = lab.trim(); const nl = norm(t);
    if (nl === "total" || nl === "draws" || nl.startsWith("pledged to") || nl === "non-pledged" || nl === "nonpledged") continue;
  ...
  const nz = monthlyTot.map((tt, k) => (tt > 0.0001 ? k : -1)).filter((k) => k >= 0);
...
/* Find the 0-indexed array row of the first label row matching `test` at/after `from`. */
function findRow(A, labelCol, test, from) {
  for (let r = from || 0; r < A.length; r++) {
    const lab = (A[r] || [])[labelCol];
    if (typeof lab === "string" && lab.trim() && test(norm(lab.trim()))) return r;
  }
  return -1;
}

function parseOrigination(wb) {
  const ws = wb.Sheets["Draws"] || wb.Sheets["Sheet3"] || wb.Sheets["Portfolio Draws"];
  if (!ws) return { months: [], borrowers: [], rows: [] };
  const A = XLSX.utils.sheet_to_json(ws, { header: 1, defval: null });
  const G = drawGeometry(A);
  if (!G) return { months: [], borrowers: [], rows: [] };
  // Main Draw Forecast = first borrower block, bounded by its first TOTAL row so the
  // per-lender section (which repeats borrower names) is never double-counted.
  const firstTotal = findRow(A, G.labelCol, (n) => n === "total", G.dateRow + 1);
  const end = firstTotal >= 0 ? firstTotal : A.length;
  return accumulateDraws(A, G, G.dateRow + 1, end);
}

/* Per-lender Draw Forecast: three independent blocks delimited by their header rows
   and their own TOTAL rows. Returns { variant, covalto, unassigned } each shaped like
   the main origination object so the dashboard can render them with identical visuals. */
function parseAssignment(wb) {
  const ws = wb.Sheets["Draws"] || wb.Sheets["Sheet3"] || wb.Sheets["Portfolio Draws"];
  if (!ws) return null;
  const A = XLSX.utils.sheet_to_json(ws, { header: 1, defval: null });
  const G = drawGeometry(A);
  if (!G) return null;
  const block = (headerTest) => {
    const h = findRow(A, G.labelCol, headerTest, G.dateRow + 1);
    if (h < 0) return { months: [], borrowers: [], rows: [] };
    const tot = findRow(A, G.labelCol, (n) => n === "total", h + 1);
    const stop = tot >= 0 ? tot : A.length;
    return accumulateDraws(A, G, h + 1, stop);
  };
  const variant = block((n) => n.includes("pledged to variant"));
  const covalto = block((n) => n.includes("pledged to covalto"));
  const unassigned = block((n) => n === "non-pledged" || n === "nonpledged" || n.startsWith("non-pledged") || n.startsWith("nonpledged"));
  return { variant, covalto, unassigned };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    try { out.assignment = parseAssignment(wb); } catch (e) { out.assignmentError = String(e.message || e); }
